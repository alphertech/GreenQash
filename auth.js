...
// File: supabase/functions/login-redirect/index.js
// Handles login validation, confirms the user exists in auth.users,
// and redirects based on whether a profile row is present.

import { createClient } from "npm:@supabase/supabase-js@2.39.2";

// Supabase client using the service‑role key (full DB access)
// Duplicate supabase declaration removed
...
Deno.serve(async (req) => {
    // Accept only POST or GET (adjust if your client uses a different verb)
    if (req.method !== "POST" && req.method !== "GET") {
        return new Response("Method not allowed", { status: 405 });
    }

    // 1️⃣ Pull JWT from the request
    const token = getBearerToken(req);
    if (!token) {
        return new Response("Missing Authorization token", { status: 401 });
    }

    // 2️⃣ Verify token – this also confirms the user exists in auth.users
    const { data: authUser, error: authErr } = await supabase.auth.getUser(token);
    if (authErr || !authUser) {
        return new Response("Invalid or expired token", { status: 401 });
    }

    // 3️⃣ (Optional) Double‑check the user row still exists in auth.users
    const { data: authRow, error: authRowErr } = await supabase
        .from("auth.users")
        .select("id")
        .eq("id", authUser.id)
        .single();

    if (authRowErr || !authRow) {
        return new Response("User not found in authentication table", { status: 401 });
    }

    // 4️⃣ Look for a profile linked to this user
    const { data: profile, error: profileErr } = await supabase
        .from("profiles")
        .select("id")
        .eq("user_id", authUser.id) // column that links profile → auth.users
        .single();
